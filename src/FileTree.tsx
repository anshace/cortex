...
type TreeNode = { name: string; children: Map<string, TreeNode>; file?: FileRow };
...
function visibleOrder(node: TreeNode, parent: string, collapsed: Set<string>, out: number[]) {
...
export type FileTreeHandle = { startCreate: (kind: "file" | "folder") => void };
...
type Props = {
  ...
  onUploadFiles: (dir: string, files: FileList) => void;
};
...
const FileTree = forwardRef<FileTreeHandle, Props>(function FileTree(props, ref) {
  const { files, rootName, onMove, onDelete, onCreate, onUpload, onUploadFiles, onToggle, collapsed } = props;
  ...
  const selectedFiles = () =>
    Array.from(selected).map((id) => byId.get(id)).filter((f): f is FileRow => !!f);

  function descendantsOf(folderPath: string): FileRow[] {
    return files.filter((f) => f.path === folderPath || f.path.startsWith(folderPath + "/"));
  }
  ...
  function commitRename(node: TreeNode, parentPath: string, name: string) {
    ...
    else for (const f of descendantsOf(oldPath)) onMove(f.id, newPath + f.path.slice(oldPath.length));
    setEditing(null);
  }
  ...
  function fileMenu(e: MouseEvent, node: TreeNode, parentPath: string) {
    ...
    setMenu({
      ...
      actions: multi
        ? [
            { label: "Download", icon: VscFile, onClick: () => targets.forEach(props.onDownload) },
            {
              label: `Delete ${targets.length} files`,
              icon: VscTrash,
              danger: true,
              divider: true,
        ...
        : [
            { label: "Open", icon: VscFile, onClick: () => props.onOpen(f) },
            { label: "Rename", icon: VscEdit, onClick: () => setEditing({ path, initial: node.name }) },
            { label: "Download", icon: VscFile, onClick: () => props.onDownload(f) },
            { label: "Delete", icon: VscTrash, danger: true, divider: true, onClick: () => onDelete([f], f.path) },
          ],
    });
  }
  ...
  function folderMenu(e: MouseEvent, folderPath: string, name: string) {
    ...
    setMenu({
      ...
      y: e.clientY,
      actions: [
        { label: "New File", icon: VscNewFile, onClick: () => startCreateIn(folderPath, "file") },
        { label: "New Folder", icon: VscNewFolder, onClick: () => startCreateIn(folderPath, "folder") },
        { label: "Upload here", icon: VscCloudUpload, onClick: () => onUpload(folderPath) },
        { label: "Rename", icon: VscEdit, divider: true, onClick: () => setEditing({ path: folderPath, initial: name }) },
        {
          label: "Delete",
          icon: VscTrash,
          danger: true,
          onClick: () => onDelete(descendantsOf(folderPath), `folder "${name}"`),
        },
      ],
    });
  }
  ...
  function rootMenu(e: MouseEvent) {
    ...
    setMenu({
      ...
      y: e.clientY,
      actions: [
        { label: "New File", icon: VscNewFile, onClick: () => startCreateIn("", "file") },
        { label: "New Folder", icon: VscNewFolder, onClick: () => startCreateIn("", "folder") },
        { label: "Upload", icon: VscCloudUpload, onClick: () => onUpload("") },
        {
          label: "Select all",
  ...
  function drop(targetPath: string, raw: string) {
    ...
    if (p.kind === "files" && p.ids) {
      ...
      const nm = F.split("/").pop() ?? F;
      const newFolder = targetPath ? `${targetPath}/${nm}` : nm;
      if (newFolder === F) return;
      for (const f of descendantsOf(F)) onMove(f.id, newFolder + f.path.slice(F.length));
    }
  }
    ...
    <>
      <Box
        ...
        onDrop={(e) => {
          ...
          else drop("", e.dataTransfer.getData("text/plain"));
        }}
        minH="140px"
        fontSize="13px"
        onClick={(e) => {
      ...
      >
        ...
        <RowShell depth={0}>
          <HStack
            ...
            onContextMenu={rootMenu}
          >
            <Icon as={rootOpen ? VscChevronDown : VscChevronRight} boxSize={`${CHEV}px`} color="ink.subtle" flexShrink={0} />
            <Text fontWeight={700} textTransform="uppercase" fontSize="11px" letterSpacing="0.04em" isTruncated>
              {rootName}
            </Text>
          </HStack>
        </RowShell>
        ...
        {rootOpen && (
          <>
            {creating && creating.parent === "" && (
              <InlineInput depth={1} kind={creating.kind} onCommit={commitCreate} onCancel={() => setCreating(null)} />
            )}
            {sorted(root).map((n) => (
              <TreeItem key={n.name} node={n} parentPath="" depth={1} {...shared} />
            ))}
          </>
        )}
      </Box>
...
function TreeItem(props: ItemShared & { node: TreeNode; parentPath: string; depth: number }) {
  const {
    node,
    parentPath,
    depth,
    ...
    return (
      <RowShell depth={depth}>
        ...
        >
          ...
          <Icon as={fi.icon} color={fi.color} fontSize="sm" flexShrink={0} mr={1.5} />
    ...
    <Box>
        ...
        {isRenaming ? (
          ...
          <HStack
            ...
            onDrop={(e) => {
              ...
              setOver(false);
              clearExternal();
              if (e.dataTransfer.files?.length) onUploadFiles(folderPath, e.dataTransfer.files);
              else drop(folderPath, e.dataTransfer.getData("text/plain"));
            }}
            pl={`${padL}px`}
            pr={2}
            py={0.5}
            ...
            _hover={{ bg: over ? "accent.tint" : "surface.hover", color: "ink.base" }}
          ...
          >
            <Icon
              ...
              flexShrink={0}
            />
            <Icon as={fic.icon} fontSize="sm" color={fic.color} flexShrink={0} mr={1.5} />
      ...
      {open && (
        <>
          ...
          {sorted(node).map((c) => (
            <TreeItem key={c.name} {...props} node={c} parentPath={folderPath} depth={depth + 1} />
          ))}
        </>
      )}
    </Box>
...
function InlineInput({
  ...
  return (
    <HStack pl={`${BASE + depth * INDENT}px`} pr={2} py={0.5} spacing={0}>
      <Box w={`${CHEV}px`} flexShrink={0} />
      <Icon as={icon} fontSize="sm" color={kind === "folder" ? "#c6923e" : "ink.subtle"} flexShrink={0} mr={1.5} />
      <Input
        autoFocus
        size="xs"
        value={v}
